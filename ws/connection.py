...
class SendEvent:
    ...
    Список событий, которые приложение может отправлять.

    ACCEPT - Отправляется приложением, когда оно желает принять входящее соединение.
    SEND - Отправляется приложением для отправки сообщения с данными клиенту.
    CLOSE - Отправляется приложением, чтобы сообщить серверу, о закрытии соединения.
Если это отправлено до того, как сокет будет принят, сервер должен закрыть     соединение с кодом ошибки HTTP 403 (Запрещено), и не завершена передача данных через вебсокет; в некоторых браузерах это может выглядеть как другой код ошибки вебсокета (например, 1006, Неправельное Закрытие (Abnormal Closure)).
    """
...
class ReceiveEvent:
    ...
    Перечисляет события, которые приложение может получать от сервера протоколов.


    CONNECT - Отправляется приложению, когда клиент изначально открывает соединение и   собирается закончить соединение вебсокета. на это сообщение необходимо ответить либо сообщением Accept, либо сообщением Close, прежде чем сокет передаст сообщения websocket.receive.
    RECEIVE - Отправляется в приложение при получении сообщения с данными от клиента.
    DISCONNECT - Отправляется приложению при потере соединения с клиентом, либо от клиента, закрывающего соединение, либо от сервера, закрывающего соединение, либо от потери сокета.
    """
